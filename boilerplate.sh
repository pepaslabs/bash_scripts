...
# boilerplate.sh: bash boilerplate
...
# demo / usage of the above.  delete this and add your implementation.

echo '$_dir: the path where this script resides:' "${_dir}"
echo '$_script: the name of this script:' "${_script}"

tempfile=$(mktempfile)
add_volatile "${tempfile}"
echo '$tempfile: a temporary file which will automatically be deleted:' "${tempfile}"

tempdir=$(mktempdir)
add_volatile "${tempdir}"
echo '$tempdir: a temporary directory which will automatically be deleted:' "${tempdir}"
